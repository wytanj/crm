...
Added the first generic profile-pack slice for Open Spine CRM. The implementation keeps the CRM vendor agnostic, headless, and agent-readable by modelling skincare as an installable pack rather than as permanent customer columns or POS-specific schema.
...
- Added built-in `skincare` and `fashion_fit` pack definitions to prove non-skincare extensibility.
...
- Added a generic Graph detail profile-pack panel that renders from definitions instead of hardcoded skincare keys.
