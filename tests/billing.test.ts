...
describe('hosted signup checkout behavior', () => {
  const payload = {
  ...
  it('keeps real billing behind an explicit integration boundary', () => {
    const response = createCheckoutResponse(payload, {
      billingMode: 'live',
    ...
    expect(response.checkoutUrl).toContain('mailto:sales@example.com')
    expect(response.checkoutUrl).toContain('Hosted%20CRM%20signup')
  })
})
